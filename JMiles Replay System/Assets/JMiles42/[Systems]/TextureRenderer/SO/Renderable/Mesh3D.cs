...
namespace JMiles42.Systems.TextureRenderer
{
    ...
    public class Mesh3D: RenderableScriptableObject
    {
        ...
        private void DrawUnityMesh(Matrix4by4 transform)
        {
            ...
            {
                ScanBuffer.FillTriangleM(new Vertex((Mosh.vertices[Mosh.triangles[i]] * RenderScale * 0.5f) + (Vector3.forward * middle.z)).
                                                 Transform(transform),
                                         new Vertex((Mosh.vertices[Mosh.triangles[i + 1]] * RenderScale * 0.5f) +
                                                    (Vector3.forward * middle.z)).Transform(transform),
                                         new Vertex((Mosh.vertices[Mosh.triangles[i + 2]] * RenderScale * 0.5f) +
                                                    (Vector3.forward * middle.z)).Transform(transform),
                                         RenderColour);
            }
        }
